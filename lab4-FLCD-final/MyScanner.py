...
class MyScanner:
    def __init__(self, file_path: str):
        self.identifier_fa = FiniteAutomaton("InOut/IdentifierFA.txt")
        self.integer_fa = FiniteAutomaton("InOut/IntegerConstantFA.txt")
        self.operators = ["+", "-", "*", "/", "%", "<=", ">=", "==", "!=", "<", ">", "<-"]
        self.separators = ["{", "}", "/", "[", "]", ":", ".", " ", ",", "\t", "\n", "'", "\""]
        self.reserved_words = [
            "mod", "add", "sub", "div", "read", "write", "if", "else", "for", "while", "int", "string", "char",
            "return", "array", "do", "show", "insert_by_user", "and", "or", "float", "range", '"-inf"',  '"inf"', "in"
        ]
    ...
    def is_identifier(self, token: str) -> bool:
        return self.identifier_fa.accepts_sequence(token)

    def is_integer_constant(self, token: str) -> bool:
        return self.integer_fa.accepts_sequence(token)
